...
const multiversionsColor = multiversions.map((el) => Object.values(el.values).map((el) =>  el.name))
...
const products = multiversions.map((el) => {
   return Object.values(el.products);
    ...
    const productUrl = products.map((el) =>  Object.values(el).map((el) => el.url)
    )
...
document.querySelector("#popUpBox").insertAdjacentHTML("afterbegin", `
